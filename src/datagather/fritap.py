...
class FriTap(DataGather):
    ...
    def start(self):
        ...
        self.job_id = self.job_manager.start_job(self.frida_script_path, custom_hooking_handler_name=self.ssl_log.on_fritap_message)
    ...
    def gather(self):
        ...
        if self.running:
            ...
            self.job_manager.setup_frida_session(self.app_package, self.profiler.on_appProfiling_message)
            #self.logger.warning("Next: start job")
            job = self.job_manager.start_job(self.frida_script_path, custom_hooking_handler_name=self.profiler.on_appProfiling_message)
            self.running = True
