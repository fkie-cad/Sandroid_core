...
from src.utils.CustomLoggerFormatter import CustomFormatter
...
class Toolbox:
    ...
    def _fetch_changed_files(cls, fetch_all=False,):
        ...
        for line in filesystem.splitlines():
            ...
            if match is None:  # Line has no time (aka is not a file)
                ...
                if secondsTimestamp > newestchange:
                    newestchange = secondsTimestamp
    ...
    def set_unset_proxy(cls):
        ...
        cls.logger.info(f"Enter proxy port (default: 8080)")
...
{spotlight_files_string}
    ...
    * {Fore.LIGHTMAGENTA_EX}[{Style.BRIGHT}q{Style.RESET_ALL}{Fore.LIGHTMAGENTA_EX}]{Fore.RESET}uit""","Sandroid Interactive Menu"))
    ...
    def exclude_whitelist(cls, file_paths):
        ...
        if cls.args.whitelist:
            ...
            if cls.file_paths_whitelist is None:
                with open(cls.args.whitelist, 'r') as f:
    ...
    def _screenrecorder_thread(cls):
        ...
        try:
            device_path = f"sdcard/screenrecord.webm"
