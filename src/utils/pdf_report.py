...
class PDFReport:
    ...
    def __init__(self, path, result_file_to_parse):
        ...
        self.small_title_font_color = Color((12.0/255), (154.0/255), (124.0/255), 1)
        self.table_line_color = Color((234.0/255), (210.0/255), (172.0/255), 1)
        self.table_header_background_color = Color((31.0/255), (32.0/255), (65.0/255), 1)
        self.white = Color(1, 1, 1, 1)

        self.firstPage()
        self.summaryTableMaker()
        if "Changed Files" in self.data.keys(): self.timelinePage()
    ...
    def summaryTableMaker(self):
        ...
        for section in self.data:
            if section in ["Device Name", "Emulator relative action timestamp", "Action Duration", "Other Data", "AI Action Summary", "AI Action Overview"]:
                continue
        ...
        for section in self.data["Other Data"]:
            ...
            if section in ["Artifact Hashes"]:
                len_new = len(self.data["Other Data"]["Artifact Hashes"][0]["new_file_hashes"])
                len_changed = len(self.data["Other Data"]["Artifact Hashes"][0]["changed_file_hashes(old,new)"])
    ...
    def image_grid(self):
        ...
        for i in range(0, len(img_list), images_per_row):
            ...
            image_row = [Image(img_dir+img_path, width=new_width*inch, height=new_height*inch) for img_path in img_list[i:i+images_per_row]]
    ...
    def newFileHashesTableMaker(self):
            ...
            new_file_hashes = self.data["Other Data"]["Artifact Hashes"][0]["new_file_hashes"]
    ...
    def changedFileHashesTableMaker(self):
            ...
            # Initialize data list with headers
            table = [self.make_header(["No.", "File Name", "Hash Before Action", "Hash After Action"])]

            # Populate data from JSON
            # with open('hashes.json', 'r') as f:
            #     json_data = json.load(f)
            #     changed_file_hashes = json_data.get("changed_file_hashes(old,new)")
            changed_file_hashes = self.data["Other Data"]["Artifact Hashes"][0]["changed_file_hashes(old,new)"]
            ...
            for file_name, hash_list in changed_file_hashes.items():
                ...
                # Add row to table
                table.append([index_cell, file_name_cell, before_hash_cell, after_hash_cell])
                lineNum += 1
